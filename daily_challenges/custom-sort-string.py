""" 
    Problem statement:
        ...
        Note:
            * String s can contain duplicate characters.
            * We need to sort the characters in s so that it satisfies the order
"""
...
class Solution:
    def customSortString(self, order: str, s: str) -> str:
        # 1. Count the occurence of character in s
        freq = dict(Counter(s))
        print(freq)
        # 2. Construct the new character based on the order of them in the order string
        rs = ""
        for char in order:
            # Find it from the freq 
            char_freq = freq.get(char, 0)
            if char_freq != 0:
                rs += char_freq * char
                # Remove that character from the freq
                del freq[char]
