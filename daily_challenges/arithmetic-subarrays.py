""" 
    Problem statement:
        A sequence of numbers is called arithmetric if it consists of at least two elements, and the difference between every two 
        ...
        Hints:
            ...
            * If and only if a set of numbers can make an arimetic sequence, then its sorted version makes an arihmetic sequence. So to check
            a set of numbers, sort it, and check if that sequence is arithmetic.
...
class Solution:
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        ...
        for i in range(len(l)):
            left, right = l[i], r[i]
            sub_sequence = nums[left: right + 1]
            sub_sequence.sort()
            rs.append(is_arithmetic(seq=sub_sequence))
        return rs
