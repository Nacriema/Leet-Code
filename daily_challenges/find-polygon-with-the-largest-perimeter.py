"""
    Problem statement:
        ...
        Example 2:
            ...
            We cannot have a polygon with either 12 or 50 as the longest side because it is not possible to include 
            2 or more smaller sides that have a greater sum than either of them.
        ...
        Hints:
            * Sort the array
            * Use greedy algorithm. If we select an edges as the longest side, it is always better to pickup all the edges with length
            no longer than this longest edge.
...
class Solution:
    def largestPerimeter(self, nums: List[int]) -> int:
        ...
        i = len(nums) - 1
        while i >= 2:
            if sum(nums[:i - len(nums)]) > nums[i - len(nums)]:
                return sum(nums[:i - len(nums)]) + nums[i - len(nums)]
            i -= 1 
        return -1
...
if __name__ == '__main__':
    ...
    print(s.largestPerimeter(nums = [5,5,50]))